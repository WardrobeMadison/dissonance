...
Saccade = Params(
    ...
    splits=["holdingpotential", "genotype", "lightmean", "celltype", "cellname"],
)
...
GA1SpikesFamily = Params(
    ...
    filter_path=Path("~/Projects/analysis/filters/GA1ledpulsefamilyspikes/GA1LedPulseFamilySpikes.txt"),
    ...
    splits=["led", "holdingpotential", "genotype", "lightmean", "celltype", "cellname", "lightamplitude"],
)

GA1Excitation = Params(
    name="GA1Excitation",
    filter_path=Path(
        "~/Projects/analysis/filters/GA1ledpulseexcitation/GA1_bothLED_nobadepochsUNFINISHED.txt"
    ),  # rod and cone
    protocols=["LedPulse"],
    paramnames=[
        "led",
...
GA1ExcitationFamily = Params(
    ...
    filter_path=Path(
        "~/Projects/analysis/filters/GA1ledpulsefamilyexcitation/GA1ledpulsefamilyexcitation.txt"
    ),  # rod and cone
    protocols=["LedPulseFamily"],
    paramnames=[
        "led",
...
Weber = Params(
    name="Weber",
    filter_path=Path("~/Projects/analysis/filters/weber/gg2_Weber.txt"),
    protocols=["LedPulseFamily"],
    paramnames=[
        "led",
        "protocolname",
...
# %% Since switching to windows, the plots show up in interactive with black background and are unreadable


@pytest.mark.parametrize(
    "folders,gui_params", parameters, ids=[f"{p.name}: {(', '.join(f))}" for f, p in parameters]
)
...
def get_paths(folders):
    paths = []
    for fldr in folders:
        paths.extend([file for ii, file in enumerate((MAP_DIR / fldr).glob("*.h5"))])
    return paths
