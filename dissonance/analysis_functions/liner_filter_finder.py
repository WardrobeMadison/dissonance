...
def linear_filter_finder(signal, response, samplerate, freqcutoff):
    ...
    filterfft = np.mean(fft(response) * np.conj(fft(signal)), axis=0) / np.mean(
        fft(signal) * np.conj(fft(signal)), axis=0
    )

    # NOTE adjust the freq cutoff for the length
    freqcutoff_adjusted = int(freqcutoff / (samplerate / len(signal)))
