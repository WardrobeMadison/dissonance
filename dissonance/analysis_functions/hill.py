...
class HillEquation:
    ...
    def __init__(self, expnt: float = None, base: float = None, rmax: float = None, xhalf: float = None):
    ...
    def __call__(self, x):
        ...
        return self.equation(x, self.expnt, self.base, self.rmax, self.xhalf)

    def fit(self, X, Y, bounds=(0.0, np.inf), maxfev=20000, **kwargs):
        p0 = [2.0, np.mean(Y), max(Y), 0.5]
        fit = curve_fit(self.equation, xdata=X, ydata=Y, maxfev=maxfev, p0=p0, bounds=bounds, **kwargs)
