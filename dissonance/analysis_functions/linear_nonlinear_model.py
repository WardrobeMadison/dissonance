...
class LinearNonLinearModel:
    ...
    def __init__(
        ...
        stimulus, lightmean, response = self.stage_inputs(stimulus, lightmean, prepoints)

        self.lf = LinearFilterModel(
            stimulus, response, prepoints, datapoints, tailpoints, samplerate, freqcutoff
        )

        self.nf = NonLinearModel(self.lf.prediction, response, prepoints, datapoints, tailpoints, numbins)
...
class LinearFilterModel:
    ...
    def __init__(self, stimulus, response, prepoints, datapoints, tailpoints, samplerate, freqcutoff):

        self.calculate_filter(stimulus, response, prepoints, datapoints, tailpoints, samplerate, freqcutoff)
    ...
    def calculate_filter(self, stimulus, response, prepoints, datapoints, tailpoints, samplerate, freqcutoff):
        ...
        self.prediction = np.empty(stimulus[:, prepoints + 1 : datapoints - (tailpoints + 1)].shape)
        self.prediction[:] = np.nan

        for ii in range(1, response.shape[0]):
            self.prediction[ii, :] = np.real(
                ifft((fft(stimulus[ii, prepoints + 1 : datapoints - (tailpoints + 1)]) * fft(self.filter)))
            )
    ...
    def measure(self, linearfilter, samplerate):
        ...
        self.time_inpeak = (linearfilter == self.inpeak) * (1 / samplerate)  # units of seconds
        ...
        if abs(self.inpeak) >= abs(self.outpeak):
            ...
            beforepeak[:, len(beforepeak), (np.floor(self.time_inpeak * samplerate)) :] = np.nan
        ...
        # off cells
            ...
            beforepeak = linearfilter
            beforepeak[:, len(beforepeak) : np.floor(self.time_outpeak * samplerate)] = np.nan
    ...
    def ampltude_spectrum_of_lin_filter(self, linearfilter, samplerate):
        ...
        binlimits = np.linspace(min(self.freqaxis), max(self.freqaxis), nyquist)  # 1 hz bins
        ...
        for ii in range(numbinlimits):
            indices = self.freqaxis > binlimits(ii) & self.freqaxis <= binlimits(ii + 1)
            self.ampspec_mean[ii] = np.mean(ampspec_filter_row(indices))
            self.ampspec_sem[ii] = np.std(ampspec_filter_row(indices)) / np.sqrt(len(indices))
...
class NonLinearModel:
    ...
    def __init__(self, linearprediction, response, prepoints, datapoints, tailpoints, numbins):
        ...
        self.mean, self.sem = self.fit_nonlinearity(self.linpred_mean, self.responses_mean)
    ...
    @staticmethod
    def static_nonlinearity(
        linearprediction, numepochs, response, prepoints, datapoints, tailpoints, numbins
    ):
        """turn linear prediction and responses into row vectors (so can bin using find)"""

        linearpredictionrow = linearprediction.reshape(1, linearprediction.shape[1] * numepochs)

        responserow = (
            response[:, prepoints + 1 : datapoints - (tailpoints + 1)].reshape(
                1, len(np.range(prepoints + 1, datapoints - (tailpoints + 1)))
            )
            * numepochs
        )

        stepsize = 100 / numbins
        binlimits = np.percentile(linearpredictionrow, np.arange(0, 100, stepsize), interpolate="midpoint")
        ...
        for ii in range(1, numbinlimits):
            indices = linearprediction > binlimits(ii) & linearprediction <= binlimits(ii + 1)

            responses_mean[ii] = np.mean(responserow(indices))
            responses_sem[ii] = np.std(responserow(indices)) / np.sqrt(len(indices))
            linpred_mean[ii] = np.mean(linearpredictionrow(indices))
            linpred_sem[ii] = np.std(linearpredictionrow(indices)) / np.sqrt(len(indices))
    ...
    def fit_nonlinearity(self, binned_linpred_mean, binned_responses_mean):
        ...
        def nrcf(z):
            return norm.ppf(
                binned_responses_mean - self.shift_cdf_function(z, binned_linpred_mean, binned_responses_mean)
            )

        # estimate parameters
        coeff = fmin(nrcf, x0=[np.mean(binned_linpred_mean), np.std(binned_linpred_mean)])
